...
EOF
        ...
        ;;
    12*)
        echo "deb http://deb.debian.org/debian/ ${DEBVER} contrib non-free-firmware" >/etc/apt/sources.list.d/contrib-non-free.list
        test -f /var/lib/apt/lists/deb.debian.org_debian_dists_${DEBVER}_non-free-firmware_binary-amd64_Packages || apt-get update
...
debootstrap --include=openssh-server,console-setup,locales,linux-headers-amd64,linux-image-amd64 \
    ...
    $DEBVER /target http://deb.debian.org/debian/

# Configure APT sources
echo "Configuring APT sources..."
cat << EOF > /target/etc/apt/sources.list
deb http://deb.debian.org/debian ${DEBVER} main contrib non-free-firmware
deb-src http://deb.debian.org/debian ${DEBVER} main contrib non-free-firmware

deb http://deb.debian.org/debian-security ${DEBVER}-security main contrib non-free-firmware
deb-src http://deb.debian.org/debian-security ${DEBVER}-security main contrib non-free-firmware

deb http://deb.debian.org/debian ${DEBVER}-updates main contrib non-free-firmware
deb-src http://deb.debian.org/debian ${DEBVER}-updates main contrib non-free-firmware
EOF
...
if [ "$USE_EFI" = true ]; then
    ...
    for i in ${!EFIPARTITIONS[@]}; do
        ...
        chroot /target /usr/sbin/grub-install --target=x86_64-efi \
            --efi-directory=/boot/efi --bootloader-id="Debian $DEBVER (disk $i)" \
            --recheck --no-floppy
        umount $EFIPARTITION
